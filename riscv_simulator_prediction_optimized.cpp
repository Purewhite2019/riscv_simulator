...
class BranchPredictor{
    ...
    virtual bool predict(int32_t) = 0;
    virtual bool lastPrediction() = 0;
    virtual void feedback(bool) = 0;
    virtual void pushPC(int32_t) = 0;
    virtual int32_t popPC() = 0;
    virtual void showAccuracy() = 0;
};

class SaturatingCounter_2bit: public BranchPredictor{
    private:
...
class TwoLevelAdaptiveBranchPredictor_Local: public BranchPredictor{
    private:
        const int BUFFERSIZE;
        int HitCount, TotCount;
        int8_t *history, **buffer;
        int32_t Last_PC, Another_PC;
        bool Last_Prediction;
    public:
        TwoLevelAdaptiveBranchPredictor_Local(int BSIZE = 0xfff): BUFFERSIZE(BSIZE){
            history = new int8_t[BUFFERSIZE];
            buffer = new int8_t*[BUFFERSIZE];
            for(int i = 0; i < BUFFERSIZE; ++i)
                buffer[i] = new int8_t[4];
            clear();
        }
        ~TwoLevelAdaptiveBranchPredictor_Local(){
            delete [] history;
            for(int i = 0; i < BUFFERSIZE; ++i)
                delete [] buffer[i];
            delete [] buffer;
        ...
        void clear() {
            ...
            Another_PC = 0;
            Last_Prediction = true;
            memset(history, 1, BUFFERSIZE*sizeof(int8_t));
            for(int i = 0; i < BUFFERSIZE; ++i)
                memset(buffer[i], 1, 4*sizeof(int8_t));
        }
        bool predict(int32_t addr) {
            Last_PC = addr;
            return (Last_Prediction = (buffer[addr&BUFFERSIZE][history[addr&BUFFERSIZE]&0b11] > 1));
        }
        bool lastPrediction() {
        ...
        void feedback(bool Fact) {
            ...
            if(Fact && buffer[Last_PC&BUFFERSIZE][history[Last_PC&BUFFERSIZE]&0b11] < 3)   // 00, 01, 10->++; 11->~
                    ++buffer[Last_PC&BUFFERSIZE][history[Last_PC&BUFFERSIZE]&0b11];

            if(!Fact && buffer[Last_PC&BUFFERSIZE][history[Last_PC&BUFFERSIZE]&0b11] > 0)   // 01, 10, 11->--; 00->~
                --buffer[Last_PC&BUFFERSIZE][history[Last_PC&BUFFERSIZE]&0b11];
            ++TotCount;
            history[Last_PC&BUFFERSIZE] >>= 1;
            history[Last_PC&BUFFERSIZE] |= (Fact << 1);
        }
        void pushPC(int32_t APC) {Another_PC = APC;}
        int32_t popPC() {return Another_PC;}
        void showAccuracy() {
...
class TwoLevelAdaptiveBranchPredictor_Related_Local: public BranchPredictor{
    private:
        const int BUFFERSIZE, LEVEL;
        int HitCount, TotCount;
        int8_t history, **buffer;
        int32_t Last_PC, Another_PC;
        bool Last_Prediction;
    public:
        TwoLevelAdaptiveBranchPredictor_Related_Local(int BSIZE = 0xff, int level = 2): BUFFERSIZE(BSIZE), LEVEL(level){
            buffer = new int8_t*[BUFFERSIZE];
            for(int i = 0; i < BUFFERSIZE; ++i)
                buffer[i] = new int8_t[1<<(LEVEL+1)];
            clear();
        }
        ~TwoLevelAdaptiveBranchPredictor_Related_Local(){
            for(int i = 0; i < BUFFERSIZE; ++i)
                delete [] buffer[i];
            delete [] buffer;
        }
        ...
        bool predict(int32_t addr) {
            Last_PC = addr;
            return (Last_Prediction = (buffer[addr&BUFFERSIZE][history] > 1));
        }
        bool lastPrediction() {
            return Last_Prediction;
        }
        void feedback(bool Fact) {
            ...
            if(Fact && buffer[Last_PC&BUFFERSIZE][history] < 3)   // 00, 01, 10->++; 11->~
                    ++buffer[Last_PC&BUFFERSIZE][history];

            if(!Fact && buffer[Last_PC&BUFFERSIZE][history] > 0)   // 01, 10, 11->--; 00->~
                --buffer[Last_PC&BUFFERSIZE][history];
            ++TotCount;
            history >>= 1;
            history |= (Fact << LEVEL);
...
class TwoLevelAdaptiveBranchPredictor_Global: public BranchPredictor{
    private:
        const int LEVEL;
        int HitCount, TotCount;
        int8_t history, *buffer;
        ...
        bool Last_Prediction;
    public:
        TwoLevelAdaptiveBranchPredictor_Global(int level = 6): LEVEL(level){
            buffer = new int8_t[1<<(LEVEL+1)];
            clear();
        }
        ~TwoLevelAdaptiveBranchPredictor_Global(){
            delete [] buffer;
        }
        void clear() {
            HitCount = 0;
            ...
            Another_PC = 0;
            Last_Prediction = true;
            memset(buffer, 1, (1<<(LEVEL+1))*sizeof(int8_t));
        }
        bool predict(int32_t addr) {
            Last_PC = addr;
            return (Last_Prediction = (buffer[history] > 1));
        }
        bool lastPrediction() {
            return Last_Prediction;
        }
        void feedback(bool Fact) {
            if(Fact == Last_Prediction)
                ++HitCount;
            if(Fact && buffer[history] < 3)   // 00, 01, 10->++; 11->~
                ++buffer[history];
            if(!Fact && buffer[history] > 0)   // 01, 10, 11->--; 00->~
                --buffer[history];
            ++TotCount;
            history >>= 1;
            history |= (Fact << LEVEL);
...
class TournamentBranchPredictor: public BranchPredictor{
    private:
        SaturatingCounter_2bit sc2_predictor;
        TwoLevelAdaptiveBranchPredictor_Local local_predictor;
        TwoLevelAdaptiveBranchPredictor_Global global_predictor;
        TwoLevelAdaptiveBranchPredictor_Related_Local related_predictor;
        int TotCount = 0, HitCount = 0;
        bool LastPrediction = false;
        int32_t Another_PC = 0;
    public:
        void clear() {
            sc2_predictor.clear();
            local_predictor.clear();
            related_predictor.clear();
            global_predictor.clear();
            TotCount = 0;
            HitCount = 0;
            Another_PC = 0;
        ...
        bool predict(int32_t addr) {
            int sc2_hit = sc2_predictor.hitcount(), local_hit = local_predictor.hitcount(), global_hit = global_predictor.hitcount(), related_hit = related_predictor.hitcount();
            bool sc2_prediction = sc2_predictor.predict(addr), local_prediction = local_predictor.predict(addr), global_prediction = global_predictor.predict(addr), related_prediction = related_predictor.predict(addr);
            if(sc2_hit >= local_hit && sc2_hit >= global_hit && sc2_hit >= related_hit) return (LastPrediction = sc2_prediction);
            else if(local_hit >= sc2_hit && local_hit >= global_hit && local_hit > related_hit) return (LastPrediction = local_prediction);
            else if(global_hit >= sc2_hit && global_hit >= local_hit && global_hit >= related_hit) return (LastPrediction = global_prediction);
            else return (LastPrediction = related_prediction);
        }
        bool lastPrediction() {return LastPrediction;}
        void feedback(bool Fact) {
            if(Fact == LastPrediction) ++HitCount;
            ++TotCount;
            sc2_predictor.feedback(Fact);
            local_predictor.feedback(Fact);
            global_predictor.feedback(Fact);
            related_predictor.feedback(Fact);
        }
        void pushPC(int32_t APC) {Another_PC = APC;}
        int32_t popPC() {return Another_PC;}
        void showAccuracy() {
...
void EX(){
    ...
    switch(ex_mem_buffer.ir & 0b1111111){
        ...
        case 0b1100011: // BEQ BNE BLT BGE BLTU BGEU
            ...
            predictor.feedback(ex_mem_buffer.cond);
